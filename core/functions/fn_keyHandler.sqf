...
switch (_code) do
{
	...
	case 36:
	{
		...
		if (_shift && !_alt && !_ctrlKey) then
		{
			...
			{
				if( !(cursorTarget getVariable "Escorting") && (( cursorTarget getVariable "restrained") || cursorTarget getVariable["playerSurrender",false] || (cursorTarget getvariable["FAR_isUnconscious",0] == 1) || (animationState cursorTarget) == "Incapacitated"))then
				{
	...
	case 19:
	{
		...
		if(_shift && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained")/*&& speed cursorTarget < 1*/) then
		{
			...
			else
			{
				...
				if(cursorTarget getVariable["playerSurrender",false] || ((cursorTarget getvariable["FAR_isUnconscious",0] == 1) && (license_civ_rebel))) then {
					[] call INVO_fnc_restrainAction;
				};
			};
		};
