...
if(_diff != (parseNumber _val)) then
{
	...
	{
		hint format["%1 vous a donne %2 mais vous ne pouvez porter que %3 donc %4 vous a ete rendu.",[_from] call life_fnc_getPlayerName,_val,_diff,((parseNumber _val) - _diff)];
	...
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		hint format["%1 vous a donne %2 %3",[_from] call life_fnc_getPlayerName,_val,_type];
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
