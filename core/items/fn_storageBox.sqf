...
if (!(_house getVariable ["contentInit", false]) OR (_house getVariable["locked", true])) exitWith {hint "tu dois d'abord déverouiller cette maison avant de poser des containers";};
if(!(_house in life_vehicles)) exitWith {hint "Tu dois être à l'intérieur de ta maison."};
_containers = _house getVariable["containers",[]];


_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(count _houseCfg == 0) exitWith {}; //What the fuck happened?
if(count _containers >= (_houseCfg select 1)) exitWith {hint "Tu ne peux plus placer de container."};
...
if(_pos isEqualTo [0,0,0]) exitWith {hint "Plus de places libres pour poser le container."};
if(!([false,_boxType,1] call life_fnc_handleInv)) exitWith {};
switch (_boxType) do {
	case "storagesmall": {
		_container = "Box_IND_Grenades_F" createVehicle [0,0,0];
		_container setPosATL _pos;
		_container setVariable ["house",_house];
