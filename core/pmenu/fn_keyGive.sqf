...
if(_index == -1) then
{
	...
	{
		_owners = _vehicle getVariable ["ziptiesOwners",[]];
		_owners pushBack _unit;
		_vehicle setVariable ["ziptiesOwners",_owners,true];
	}
	else
	{
...
hint format["Tu as donné à %1 les clées de %2",[_unit] call life_fnc_getPlayerName,typeOf _vehicle];
