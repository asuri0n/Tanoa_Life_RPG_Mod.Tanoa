...
ctrlSetText[2932, format["%1", invo_sound_dim_car]];
ctrlSetText[2942, format["%1", invo_sound_dim_air]];
ctrlSetText[2952, format["%1", invo_sound_dim_sea]];

//Setup Sliders range
{ slidersetRange [_x,0,100];} foreach [2931,2941,2951];
//Setup Sliders speed
{ ((findDisplay 2900) displayCtrl _x) sliderSetSpeed [1,1,1]; } foreach [2931,2941,2951];
//Setup Sliders position
{
	sliderSetPosition[_x select 0, _x select 1];
} foreach [[2931,invo_sound_dim_car],[2941,invo_sound_dim_air],[2951,invo_sound_dim_sea]];

private["_display","_side","_tags","_objs","_intro"];
...
if(life_revealObjects) then {
	...
	_objs buttonSetAction "[LIFE_ID_RevealObjects,""onEachFrame""] call BIS_fnc_removeStackedEventHandler; life_revealObjects = false; profileNamespace setVariable [""life_revealObjects"", false]; [] call life_fnc_settingsMenu;";
} else {
	_objs ctrlSetTextColor [1,0,0,1];
	_objs ctrlSetText "OFF";
	_objs buttonSetAction "LIFE_ID_RevealObjects = [""LIFE_RevealObjects"",""onEachFrame"",""life_fnc_revealObjects""] call BIS_fnc_addStackedEventHandler; life_revealObjects = true; profileNamespace setVariable [""life_revealObjects"", true]; [] call life_fnc_settingsMenu;";
};
