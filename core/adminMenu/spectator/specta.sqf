...
//ADD INVO LOOP Lambert
INVO_loop = true;
[] spawn {
	sleep 1;
	_INVO_memPosPlayer = getPos player;
	sleep 0.2;
	player setDamage 0;
	...
	while {INVO_loop} do
	{
		if (KEGs_target != player) then		//si cible différente du joueur
		{
			_posTarget = getPos (vehicle KEGs_target);
			player setPos [_posTarget select 0, _posTarget select 1, (_posTarget select 2) + 5 ];
		};
		sleep 0.1;
	};
	player setPos _INVO_memPosPlayer;
	sleep 0.5;
	player enableSimulation true;
	sleep 0.5;
	//switch off invisibility
...
SPECTATOR_PATH = "core\adminMenu\";

spectate_events = compile preprocessFileLineNumbers format ["%1spectator\specta_events.sqf",SPECTATOR_PATH];
RefreshPlayerList = compile preprocessFileLineNumbers format ["%1spectator\RefreshPlayerList.sqf",SPECTATOR_PATH];
FreeLookMovementHandler = compile preprocessFileLineNumbers format ["%1spectator\FreeLookMovementHandler.sqf",SPECTATOR_PATH];
CameraMenuHandler = compile preprocessFileLineNumbers format ["%1spectator\CameraMenuHandler.sqf",SPECTATOR_PATH];
PlayerMenuHandler = compile preprocessFileLineNumbers format ["%1spectator\PlayerMenuHandler.sqf",SPECTATOR_PATH];
KEGsShowCombatMode = compile preprocessFileLineNumbers format ["%1spectator\specta_combatmode.sqf",SPECTATOR_PATH];
KEGsShowUnitLocator = compile preprocessFileLineNumbers format ["%1spectator\specta_locator.sqf",SPECTATOR_PATH];
...
KEGs_cameraNames = ["Verrouillée", "Poursuite", "1st-Person", "Libre"];
