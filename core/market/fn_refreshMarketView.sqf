...
else
{
...

	{
		...
		{
			ctrlSetText [1001, format["$%1", [_x select 3] call life_fnc_numberText ] ];

			//Trend Global
			ctrlSetText [1004, format["$%1", [_x select 9] call life_fnc_numberText ] ];

			if((_x select 9) >= 0) then
			{
			...
			//Trend local
			ctrlSetText [1005, format["$%1", [_x select 10] call life_fnc_numberText ] ];

			if((_x select 10) >= 0) then
			{
