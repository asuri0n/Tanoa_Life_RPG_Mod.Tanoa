...
switch (life_garage_type) do
{
    case "Car": {invo_garage_last_query_car = _vehicles; invo_garage_change_car = false;};
    case "Air": {invo_garage_last_query_Air = _vehicles; invo_garage_change_air = false;};
    case "Ship": {invo_garage_last_query_Ship = _vehicles; invo_garage_change_ship = false;};
};
...

{
	...
	_tmp = str(_tmp);
	/* diag_log format ["INVO DEBUG - impoundMenu - tmp: %1", _tmp]; */
