...
diag_log "INVO LOGS - AgentRev - vehicleManager.sqf launched.";
...

	{
		if (!(_x isKindOf "CAManBase") && _x != _R3F_attachPoint) then
		{
			...
			if (!local _vehicle &&
			   ...
			   ((getPosATL _vehicle) select 2 < 1 || {_vehicle isKindOf "Static"})}) then
			{
...
while {true} do
{
	...
	sleep 5;
	//diag_log "INVO LOGS - vehicleManager loop.";
};
